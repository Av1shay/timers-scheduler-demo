...
func (tc *TaskCreate) SetWebhookUrl(s string) *TaskCreate {
	tc.mutation.SetWebhookUrl(s)
	return tc
}
...
func (tc *TaskCreate) AddHistoryIDs(ids ...int) *TaskCreate {
	tc.mutation.AddHistoryIDs(ids...)
	return tc
}

// AddHistories adds the "histories" edges to the TaskHistory entity.
func (tc *TaskCreate) AddHistories(t ...*TaskHistory) *TaskCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
	return tc.AddHistoryIDs(ids...)
}
...
func (tc *TaskCreate) check() error {
	...
	if _, ok := tc.mutation.WebhookUrl(); !ok {
		return &ValidationError{Name: "webhookUrl", err: errors.New(`ent: missing required field "Task.webhookUrl"`)}
	}
	if _, ok := tc.mutation.Status(); !ok {
...
func (tc *TaskCreate) createSpec() (*Task, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.WebhookUrl(); ok {
	...
	if nodes := tc.mutation.HistoriesIDs(); len(nodes) > 0 {
