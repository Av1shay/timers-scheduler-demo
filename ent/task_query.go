...
func (tq *TaskQuery) QueryHistories() *TaskHistoryQuery {
	query := &TaskHistoryQuery{config: tq.config}
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withHistories: tq.withHistories.Clone(),
...
func (tq *TaskQuery) WithHistories(opts ...func(*TaskHistoryQuery)) *TaskQuery {
	query := &TaskHistoryQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withHistories = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withHistories; query != nil {
		if err := tq.loadHistories(ctx, query, nodes,
...
func (tq *TaskQuery) loadHistories(ctx context.Context, query *TaskHistoryQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskHistory)) error {
	...
	query.Where(predicate.TaskHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(task.HistoriesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.task_histories
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_histories" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_histories" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
